...
class H2OKVCache(nn.Module):
    """
    H2O code for evicting the KV cache,
    computes the HH score and selects the HH and recent tokens
    to evict past_key_values
    """
    ...
    def __init__(
            ...
            recent_ratio: float,
            hh_fixed_size: int = 1,
            recent_fixed_size: int = 1,
        ):
        super().__init__()
        self.hh_score = None
        self.hh_ratio = hh_ratio
        self.recent_ratio = recent_ratio
        self.hh_fixed_size = hh_fixed_size
        self.recent_fixed_size = recent_fixed_size
    ...
    def forward(
        ...
        """
        Args:
            past_key_values: A tuple of (key_states, value_states)
                key_states: torch.FloatTensor of shape (batch_size, num_heads, seq_len, head_dim)
                value_states: torch.FloatTensor of shape (batch_size, num_heads, seq_len, head_dim)
            attn_score_cache: torch.Tensor of shape (batch_size, num_heads, query_states, seq_len)
        """
        ...
        num_groups = attn_score_cache.shape[1] // num_heads

        self._update_hh_score(num_groups, attn_score_cache)

        seq_len = key_states.size(2) -1

        hh_size = int(self.hh_ratio * seq_len) + self.hh_fixed_size
        recent_size = int(self.recent_ratio * seq_len) + self.recent_fixed_size
        if seq_len <= hh_size + recent_size:
            return past_key_values

        select_hh_scores = self.hh_score[:, :seq_len - recent_size]
        _, keep_topk = torch.topk(select_hh_scores, hh_size, dim=-1)   # indices of topk
        keep_topk = keep_topk.sort().values

        keep_recent = torch.arange(seq_len - recent_size, seq_len, device=keep_topk.device).repeat(keep_topk.shape[0], 1)  # indices of recent
        keep_idx = torch.cat([keep_topk, keep_recent], dim=-1)  # indices to keep

        mask = torch.zeros(self.hh_score.shape, dtype=torch.bool).to(past_key_values[0].device)
        mask = mask.scatter(-1, keep_idx, 1)

        key_states_last = key_states[:, :, -1, :]
        value_states_last = value_states[:, :, -1, :]

        key_states = key_states[:, :, :-1, :].squeeze()
        value_states = value_states[:, :, :-1, :].squeeze()

        # select hh and recent tokens for key and value
        k_hh_recent = []
        v_hh_recent = []
        for i in range(key_states.size(0)):
            k_hh_recent.append(key_states[i][mask[i]])
            v_hh_recent.append(value_states[i][mask[i]])
        k_hh_recent = torch.stack(k_hh_recent).view(bsz, num_heads, -1, head_dim)
        v_hh_recent = torch.stack(v_hh_recent).view(bsz, num_heads, -1, head_dim)

        k_hh_recent = torch.cat([k_hh_recent, key_states_last.unsqueeze(2)], dim=2)
        v_hh_recent = torch.cat([v_hh_recent, value_states_last.unsqueeze(2)], dim=2)

        hh_score = []
        for i in range(self.hh_score.size(0)):
            hh_score.append(self.hh_score[i][mask[i]])
        self.hh_score = torch.stack(hh_score)

        return (k_hh_recent, v_hh_recent)
    
    def _update_hh_score(self, num_groups: int, attn_score_cache: torch.Tensor):

        bsz, num_heads, num_new_tokens, _ = attn_score_cache.shape
        assert bsz == 1

        attn_score_cache = attn_score_cache.sum(0).sum(1).reshape(num_heads // num_groups, num_groups, -1).sum(1)
...
class LlamaAttentionBase(_LlamaAttention):
    ...
    def __init__(self, config: H2OLlamaConfig):
        ...
        self.kv_cache_evict = H2OKVCache(config.heavy_ratio, config.recent_ratio)
    ...
    def _forward_decoding(
        ...
        **kwargs,
        ...
        # H2O
        past_key_value = self.kv_cache_evict(past_key_value, attn_weights.detach().clone())
        ...
        **kwargs,
        ...
        Args:
            ...
            attention_mask (`torch.FloatTensor`, *optional*):
                attention mask of size `(batch_size, 1,
...
class LlamaModel(_LlamaModel):
    ...
    def forward(
        ...
        if attention_mask is not None:
            attention_mask.unsqueeze_(0).unsqueeze_(1)
